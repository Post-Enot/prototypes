...
namespace IUP.Toolkits.BattleSystem
{
    public sealed class BattleArenaGenerator : IBattleArenaGenerator
    {
        ...
        {
            var battleArena = new BattleArena(arenaPattern.Map.Width, arenaPattern.Map.Height);
            ...
            {
                for (int y = 0; y < arenaPattern.Map.Width; y += 1)
                {
                    for (int x = 0; x < arenaPattern.Map.Height; x += 1)
                    {
